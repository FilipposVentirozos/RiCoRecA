...
class ActionSet:
    ...
    action_sets = None
    # or_dict = ReversibleDict()
    # def __repr__(self):
    #     if self.anchor_code:
    #         print(recipe["text"][self.anchor_code.span["start"]: self.anchor_code.span["end"]])
    ...
    def get_masters(self, member=True, depend=True, join_=True, or_=True):
        ...
        #     continue_ = True
        # Get all the ActionSets masters
    ...
    def remove_parents_junction_nodes(self, masters):
        ...
        :return:
        """
        # if action_set_pair in list(masters__.values()):
        # for pos, action_set in masters.items():

        # Check in the dependency chain if there is a Member, which itself is not linked due to Annotator error
            ...
            for relation in ActionSet.recipe["relations"]:
                ...
                if relation["child"] == action_set.anchor_code.get_id() and  ActionSet.get_action_set_position(relation["head"]):
                    ...
                    for relation_pair in action_set.relations["join"]:
                        ...
                        #     for relation_ in action_set.relations[relation_type]:
                        #         try:
                        #             action_set_ = ActionSet.action_sets[relation_["dest"][0]][relation_["dest"][1]]
                        #             masters = self.remove_parent_children(action_set_, masters)
                        #             del masters[relation_["dest"]]
                        #         except KeyError:
                        #             continue
    ...
    # todo fix me, to get the first of the Or from the dependency chain
    "(venv) chroner@z590:~/PhD_remote/FoodBase$ prodigy custom_recipe te  /home/chroner/PhD_remote/FoodBase/data/Processed/DB/Final/Lunch/Phil/prodigy.json -F recipe.py"
    ...
    # def recv_master_of(self, masters=dict()):
    #     if list(self.get_master_of()):
    #         for action_set in self.get_master_of():
    #             masters[action_set.anchor_code.get_id()] = action_set
    #             yield from action_set.recv_master_of(masters)
    #     else:
    #         yield masters
        # for action_set in self.get_master_of():
        #     yield action_set.recv_master()
    ...
    # def recv_join_or(self, masters=None, paths=None):
    #     if not masters:
    #         masters = dict()
    #     if not paths:
    #         paths = defaultdict(list)
    #     for type_, relations in self.relations.items():
    #         if type_ in ["join", "or"]:
    #             for relation in relations:
    #                 if not relation.get("dest", None):
    #                     continue
    #                 elif relation.get("dest", None) == self.get_my_position():
    #                     continue
    #                 elif relation.get("dest", None) in paths.get(self.get_my_position(), []):
    #                     continue
    #                 elif relation.get("dest", None) not in masters:
    #                     masters[relation["dest"]] = ActionSet.action_sets[relation["dest"][0]][relation["dest"][1]]
    #                     paths[self.get_my_position()].append(relation["dest"])
    #                     masters_, paths_ = masters[relation["dest"]].recv_join_or(masters=masters, paths=paths)
    #                     masters = masters | masters_
    #                     for k, v in paths_.items():
    #                         for pos in v:
    #                             if pos not in paths[k]:
    #                                 paths[k].append(pos)
    #     return masters, paths


    def fill_in_entities(self, repeat=0):
        # if not or_join_stack:
        #     or_join_stack = set()
        if not self.lu:
        ...
        if masters_join_or:  # todo check me
            ...
            #             if k in ["join", "or"]:
            #                 for relation in relations:
            #                     if relation.get("dest", None) == ActionSet.get_action_set_position(action_set):
            #                         # or_join_stack.add(action_set) # todo fill in below the recursive
            #                         # or_join_stack.add(self)
            #                         # Check first each master that has not been EntityMatched except them
            #                         masters_pair, _ = action_set.recv_masters("get_masters", masters=None, paths=None)
            #                         # Remove each other
            #                         del masters_pair[self.get_my_position()]
            #                         masters_ = copy.copy(masters)
            #                         del masters_[ActionSet.get_action_set_position(action_set)]
            #                         # Merge
            #                         # Fill in the entities that have not been matched from previous masters
            #                         merged_masters = masters_pair | masters_
            #                         for action_set_ in merged_masters.values():
            #                             if not action_set_.entity_matched:
            #                                 # raise NotEntityMatched("", action_set=action_set_)
            #                                 action_set_.fill_in_entities(repeat=repeat)
            #                         # If not
            #                         # Then chose to proceed with the one has the longest streak of masters
            #                         if len(masters_) >= len(masters_pair):
            #                             break  # Continue finding LUs
            #                         else:
            #                             # raise NotEntityMatched("", action_set=action_set)
            #                             action_set.fill_in_entities(repeat=repeat)
            #                 else:
            #                     continue
            #                 break
            #         else:
            #             # print(action_set.anchor_code.out())
            #             # If the previous ActionSet has not been entity matched, prioritise it.
            #             action_set.fill_in_entities(repeat=repeat)
            #             # raise NotEntityMatched("", action_set=action_set)

        # Enlist candidates from ActionSets that belong to it
        # for action_set in self.get_action_set_children():
        #     # There is a chance of losing the ActionSet that is Member of self and is not linked with self
        #     # but it's predecessor (i.e. master) is
        #     if not action_set.entity_matched:
        #         raise NotEntityMatched("", action_set=action_set)
        #     for lu in action_set.lu:
        #         # Do only for Entity types belong to ne.Entity and subclasses
        #         try:
        #             cands[lu.out()] = (lu.primes, lu.get_entity_type())
        #         except AttributeError:
        #             continue
        if masters:
        ...
        for lu in self.lu:
            ...
            if not lu.primes:
                ...
                if repeat < 10:
                    ...
                    # If not, due to annotation error, take the closest assigned entity
                    # Fill in the entity with the latest Cand
                    # if method_2:
                    #     max_, primes = -1, set()
                    #     for cand in cands.values():
                    #         if cand[1] == "ingr":
                    ...
                    #     else:
                    #         raise NotImplementedError
                    # Fill in with the primes that have not been assigned
                    primes = set()
                    for cand in cands.values():
